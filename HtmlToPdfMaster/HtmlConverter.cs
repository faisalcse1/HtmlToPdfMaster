...
namespace HtmlToPdfMaster
{
    ...
    public class HtmlConverter
    {
        ...
        /// <returns></returns>
        public static byte[] FromHtmlString(string html, int width, int height, int margin_left=0, int margin_top=0, int margin_right = 0 , int margin_bottom = 0, string encoding="UTF-8")
        {
            var filename = Path.Combine(directory, $"{Guid.NewGuid()}.html");
            File.WriteAllText(filename, html);
            var bytes = FromUrl(filename, width, height, margin_left, margin_top, margin_right, margin_bottom, encoding);
        ...
        /// <param name="encoding">Set the default text encoding, for input.</param>
        /// <returns></returns>
        public static byte[] FromUrl(string url, int width, int height, int margin_left = 0, int margin_top = 0, int margin_right = 0, int margin_bottom = 0, string encoding="UTF-8")
        {
